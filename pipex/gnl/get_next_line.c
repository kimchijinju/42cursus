...
/*   Updated: 2022/09/29 16:27:08 by hanbkim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*new_next_line(char **backup, t_list **head, int fd)
{
	...
	temp = *backup;
	backup_len = ft_strlen_gnl(temp);
	newline_prev = has_newline(temp);
	str = malloc(newline_prev + 1);
	if (!str)
		return (ft_list_remove_if_gnl(head, fd));
	str = ft_memmove_gnl(str, temp, newline_prev);
	str[newline_prev] = '\0';
	temp = ft_memmove_gnl(temp, temp + newline_prev,
...
char	*get_next_line(int fd)
{
	...
	if (!node->backup)
		return (ft_list_remove_if_gnl(&head, fd));
	backup_len = ft_strlen_gnl(node->backup);
	if (has_newline(node->backup))
		return (new_next_line(&(node->backup), &head, fd));
	if (*node->backup)
	{
		ret = malloc(backup_len + 1);
		ret = ft_memmove_gnl(ret, node->backup, backup_len + 1);
	}
	ft_list_remove_if_gnl(&head, fd);
	return (ret);
}
