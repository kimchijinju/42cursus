...
/*   Updated: 2022/10/27 15:17:21 by hanbkim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*read_target(int fd, char *backup)
{
	...
	{	
		...
		temp = backup;
		if (!backup)
			backup = ft_strdup_vergnl(buf);
		else
			backup = ft_strjoin_vergnl(backup, buf);
		if (!backup)
			return (NULL);
	}
	return (backup);
...
static char	*new_next_line(char **backup, t_list **head, int fd)
{
	...
	temp = *backup;
	backup_len = ft_strlen_vergnl(temp);
	newline_prev = has_newline(temp);
	str = malloc(newline_prev + 1);
	if (!str)
		return (ft_list_remove_if_vergnl(head, fd));
	str = ft_memmove_vergnl(str, temp, newline_prev);
	str[newline_prev] = '\0';
	temp = ft_memmove_vergnl(temp, temp + newline_prev,
...
char	*get_next_line(int fd)
{
	...
	if (!node->backup)
		return (ft_list_remove_if_vergnl(&head, fd));
	backup_len = ft_strlen_vergnl(node->backup);
	if (has_newline(node->backup))
		return (new_next_line(&(node->backup), &head, fd));
	if (*node->backup)
	{
		ret = malloc(backup_len + 1);
		ret = ft_memmove_vergnl(ret, node->backup, backup_len + 1);
	}
	ft_list_remove_if_vergnl(&head, fd);
	return (ret);
}
